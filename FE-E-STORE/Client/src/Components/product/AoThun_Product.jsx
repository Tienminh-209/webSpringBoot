...
class AoThun_Product extends Component {
  constructor(props) {
    super(props);
    this.state = {
      content: [],
  ...
  previousPage() {
    ...
    this.hotNew(this.state.currentPage1);
    this.hotBest(this.state.currentPage2);
  ...
  hotNew(currentPage1) {
    currentPage1 -= 1;
    ProductService.getHotNewInPageOrther(currentPage1, this.state.size)
      .then((response) => response.data)
      .then((data) => {
        this.setState({
          hotnew: data.content,
  ...
  hotBest(currentPage2) {
    currentPage2 -= 1;
    ProductService.getHotBestInPageOrther(currentPage2, this.state.size)
      .then((response) => response.data)
      .then((data) => {
        this.setState({
      ...
      <div style={{ marginLeft: "5px", marginRight: "5px" }}>
        ...
        <div className="row">
          <article className="filter-group" style={{ display: "flex" }}>
            ...
            <FilterProduct />
            <div style={{ fontSize: "15px", marginLeft: "185px",marginTop:"18px" }}>Có {this.state.content.length} sản phẩm trong trang {this.state.currentPage}</div>
          </article>
          <div className="row">
